...
function fillScene() {
    ...
    var handLeftMaterial = new THREE.MeshPhongMaterial({ color: 0xCC3399, specular: 0xCC3399, shininess: 20 });
    var handRightMaterial = new THREE.MeshPhongMaterial({ color: 0xDD3388, specular: 0xDD3388, shininess: 20 });
    createRobotGrabber(handLeft, handLength, handLeftMaterial);
    createRobotGrabber(handRight, handLength, handRightMaterial);

    forearm.add(handLeft);
    forearm.add(handRight);
}

function createRobotGrabber(parent, length, material) {
    var box = new THREE.Mesh(new THREE.BoxGeometry(30, length, 4), material);
...
function init() {
	...
	var canvasHeight = window.devicePixelRatio*window.innerHeight;
...
function render() {
 ...
 body.rotation.y = effectController.by * Math.PI/180;	// yaw
	...
	handLeft.rotation.z = effectController.hx * Math.PI/180;
	handRight.rotation.z = effectController.hx * Math.PI/180;
	handLeft.position.z = effectController.hs / 2;
	handRight.position.z = -effectController.hs / 2;
...
function setupGui() {
	...
	h.add(effectController, "hx", -120.0, 120.0, 0.025).name("Grabber x");
	h.add(effectController, "hs", 5, 30.0, 0.025).name("Grabber spread");
}
