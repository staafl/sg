...
function fillScene() {
	...
	//placeCylinder(new THREE.Vector3(1, 1, 1), new THREE.Vector3( -1,-1,-1 ));
    //placeCylinder(new THREE.Vector3(1, -1, 1), new THREE.Vector3( -1,1,-1 ));
    //placeCylinder(new THREE.Vector3(-1, -1, 1), new THREE.Vector3( 1,1,-1 ));
    //placeCylinder(new THREE.Vector3(-1, 1, 1), new THREE.Vector3( 1,-1,-1 ));
    placeCylinder(new THREE.Vector3(-1, 1, -1), new THREE.Vector3( 1,-1,-1 ));
    function placeCylinder(corner1, corner2) {
    	...
    	var cylAxis = new THREE.Vector3().subVectors( corner1, corner2 );
    	...
    	//window.alert(cylLength);

    	var cylinder = new THREE.Mesh(
    		new THREE.CylinderGeometry( 0.2, 0.2, cylLength, 32 ), cylinderMaterial );
    	var rotationAxis = yAxis.cross(cylAxis);
    	// collinear vectors?
    	if (rotationAxis.length() == 0) {
    	    // we could just skip rotation altogether since the objects are symmetric
    	    // rotationAxis = null;
    	    var dot = yAxis.dot(cylAxis);
    	    if (dot > 0) { // codirectional vectors
    	        rotationAxis = null;
    	    }
    	    else { // antidirectional vectors
    	        // theta = Math.PI;
                rotationAxis = new THREE.Vector3(1,0,0); // use x axis
    	    }
    	}
    	if (rotationAxis) {
